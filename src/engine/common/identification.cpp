...
void ID_Init( void )
{
	...
	{
		CHAR szBuf[MAX_PATH];
		ID_GetKeyData( HKEY_CURRENT_USER, "Software\\rexash\\", "xash_id", (LPBYTE)szBuf, MAX_PATH );
...
#if defined(_WIN32)
	{
		...
		ID_SetKeyData( HKEY_CURRENT_USER, "Software\\rexash\\", REG_SZ, "xash_id", (LPBYTE)Buf, Q_strlen(Buf) );
	}
#else
	{
		const char *home = getenv( "HOME" );
