...
{
	//if( !vgui.initialized ) // already have lmao
	//	return;
	...
	{
	case K_MOUSE1:
		VGUI_Mouse( down?MA_PRESSED:MA_RELEASED, MOUSE_LEFT );
		return;
	case K_MOUSE2:
		VGUI_Mouse( down?MA_PRESSED:MA_RELEASED, MOUSE_RIGHT );
		return;
	case K_MOUSE3:
		VGUI_Mouse( down?MA_PRESSED:MA_RELEASED, MOUSE_MIDDLE );
		return;
	case K_MWHEELDOWN:
		VGUI_Mouse( MA_WHEEL, 1 );
		return;
	case K_MWHEELUP:
		VGUI_Mouse( MA_WHEEL, -1 );
		return;
	...
	if( down == 2 )
		VGUI_Key( KA_TYPED, VGUI_MapKey( key ) );
	else
		VGUI_Key( down?KA_PRESSED:KA_RELEASED, VGUI_MapKey( key ) );
	//Con_Printf("VGui_KeyEvent %d %d %d\n", key, VGUI_MapKey( key ), down );
}
...
void VGui_MouseMove( int x, int y )
{
	float xscale = scr_width->value / (float)clgame.scrInfo.iWidth;
	float yscale = scr_height->value / (float)clgame.scrInfo.iHeight;
	//AVSARTODO 2348908234789234789324
	VGUI_MouseMove( x / xscale, y / yscale );
}
