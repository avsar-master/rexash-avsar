...
void CBaseUI::Initialize(CreateInterfaceFn* factories, int count) {
	...
	vgui2::filesystem()->AddSearchPath(cl_enginefuncs.pfnGetGameDirectory(), "GAME");
	vgui2::filesystem()->AddSearchPath("cstrike", "GAMECONFIG");
	vgui2::filesystem()->AddSearchPath(cl_enginefuncs.pfnGetGameDirectory(), "GAMECONFIG");
	...
	//Initalize Client.
	gClDllFuncs_F = (void(*)(void*))GetProcAddress((HMODULE)m_hClientModule, "F");
	modfuncs_t modfuncs;
	...
	gClDllFuncs.pfnInitialize(&cl_enginefuncs, CLDLL_INTERFACE_VERSION);
...
void CBaseUI::Start(struct cl_enginefuncs_s *engineFuncs, int interfaceVersion) {
	...
	const char *szGameDir = cl_enginefuncs.pfnGetGameDirectory();
	...
	{
		void* system = nullptr;
		staticGameUIFuncs->Start(&cl_enginefuncs, CLDLL_INTERFACE_VERSION, system);
	}
...
void CBaseUI::HideGameUI() {
	Key_SetKeyDest(key_game);// AVSARTODO
...
void CBaseUI::ActivateGameUI() {
	Key_SetKeyDest(key_menu); // AVSARTODO
